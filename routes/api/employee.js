...
router.post('/',
    ...
    async (req, res) => {
        ...
        const userEmp = {
            ...
            password : '123456' //keeping a default password
        }
...
router.get('/', auth , 
    async (req, res) => {
        try {
            const profile = await Employee.find().populate('user',['name','avatar']);
            res.json(profile);
        } catch (err){
...
router.put('/leave/:emp_id',
    [
        auth,
        [
            check('from','Please provide a start date')
                .notEmpty()
